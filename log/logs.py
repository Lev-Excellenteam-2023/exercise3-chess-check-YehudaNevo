...
class ChessGame:
    ...
    def initialize_board(self):
        return ['piece1', 'piece2', 'knight', 'swimmer']
    ...
    def log_move(self, piece, start_pos, end_pos):
        ...
        if piece == 'knight': self.knight_moves += 1
        logger.debug(f"[Game ID {self.game_id}] {piece} moved from {start_pos} to {end_pos}")

    def log_board_state(self):
        logger.debug(f"[Game ID {self.game_id}] Current board state: {str(self.board)}")
    ...
    def log_game_end(self, result):
        ...
        logger.info(f"[Game ID {self.game_id}] Total knight moves: {self.knight_moves}")
        logger.info(f"[Game ID {self.game_id}] Total swimmers: {self.swimmers}")
